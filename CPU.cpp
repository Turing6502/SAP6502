...
void	ExecuteBRA(unsigned char instr)
{
	...
	SetMicroInstruction(instr, (PCH << LOAD) | (ALU << DRIVER) | (ALUADC << ALUOP) | PCCLOCKHI | NOBCD | CARRYZERO | UCOUNTERRESET, 0);
	SetMicroInstruction(instr, (PCH << LOAD) | (ALU << DRIVER) | (ALUADC << ALUOP) | PCCLOCKHI | NOBCD | CARRYONE  | UCOUNTERRESET, 1);
}
...
void	ExecuteJSR (unsigned char instr)		
{
	...
	//	SPREG--;
	SetMicroInstruction(instr, (BREG << LOAD) | (EAL<< DRIVER));								//	BREG <- EAL
	SetMicroInstruction(instr, (SPREG << LOAD) | (ALU << DRIVER) | (ALUADC << ALUOP) | NOBCD | CARRYZERO);	//	SPREG <- AH + BREG
...
void	ExecuteRTS(unsigned char instr)
{
	...
	//	SPREG = EAL
	//	PC++
	SetMicroInstruction(instr, (SPREG << LOAD) | (PCL << DRIVER) | PCCLOCKLO | PCCLOCKHI | UCOUNTERRESET);	//	Really want EAL							//	SPREG <- EAL
}
...
{ 
//	SetMicroInstruction(instr, (BREG << LOAD) | (XREG << DRIVER));
//	SetMicroInstruction(instr, (SPREG << LOAD) | (ALU << DRIVER) | (ALUBREG << ALUOP) | UPDATENZ | UCOUNTERRESET);
